...
void handleError(const EtcdError& error) {
    switch (error.type()) {
        ...
        case EtcdErrorType::Send:
            std::cerr << "发送错误: " << error.message() << '\n';
            break;
        case EtcdErrorType::Recv:
            std::cerr << "接收错误: " << error.message() << '\n';
            break;
...
config.endpoint = "http://127.0.0.1:2379";
config.request_timeout = std::chrono::milliseconds(5000);  // 5 秒超时
