...
- Git Tag：`v3.0.3`
- 自述摘要：
  - 将安装导出的 CMake targets 文件改为 `galayEtcdConfigTargets.cmake`，并同步 `galay-etcd-config.cmake` 的 include 路径。
  - Release 安装随主 targets 文件生成 `galayEtcdConfigTargets-release.cmake`，统一驼峰导出文件命名。
  - 将 CMake project 版本提升到 `3.0.3`，确保源码版本元数据、tag 与发布记录一致。

## v3.1.0 - 2026-05-20

- 版本级别：中版本（minor）
- Git 提交消息：`feat: 增加 etcd 库级 BaseLogger 日志入口`
- Git Tag：`v3.1.0`
- 自述摘要：
  - 新增 `galay::etcd::log::set/get` 库级日志入口，使用 `galay-kernel` 的 `BaseLogger` 与独立 logger 槽位，允许用户只启用 galay-etcd 日志。
  - 新增 `ETCD_LOG_*` 埋点宏，并在同步/异步 etcd client 的连接、请求、响应错误和 watch worker 生命周期路径补充日志。
  - 新增 `T10-EtcdLog` 回归测试，验证未设置 logger 和日志级别过滤时不会求值格式化参数。
  - 移除旧 spdlog 日志 helper 依赖，文档改为描述 BaseLogger 注入方式。
  - 将 `galay-http` 依赖提升到 `3.1.0`，继续对齐 `galay-kernel 5.0.0`，并同步 CMake project/package 版本到 `3.1.0`。
