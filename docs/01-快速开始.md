# 01-快速开始

## 0. 依赖

- Galay 内部依赖（构建必需 + 联调推荐）：
  - [galay-kernel](https://github.com/gzj-creator/galay-kernel)（构建必需）
  - [galay-utils](https://github.com/gzj-creator/galay-utils)（推荐）
  - [galay-http](https://github.com/gzj-creator/galay-http)（推荐）
- 第三方依赖：
  - C++23 编译器
  - CMake 3.20+
  - simdjson（pkg-config）
  - spdlog

## 1. 构建

```bash
git clone https://github.com/gzj-creator/galay-kernel.git
git clone https://github.com/gzj-creator/galay-utils.git
git clone https://github.com/gzj-creator/galay-http.git
git clone https://github.com/gzj-creator/galay-etcd.git
cd galay-etcd
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build --parallel
```

仅单独构建 `galay-etcd` 时，最小内部依赖为 `galay-kernel` 和 `galay-http`。

## 2. 同步阻塞接口最小示例

```cpp
#include "galay-etcd/sync/EtcdClient.h"
#include <iostream>

int main() {
    galay::etcd::EtcdConfig cfg;
    cfg.endpoint = "http://127.0.0.1:2379";
    galay::etcd::EtcdClient session(cfg);

    auto conn = session.connect();
    if (!conn) return 1;

    auto put = session.put("/demo/key", "hello");
    if (!put) return 1;

    auto get = session.get("/demo/key");
    if (!get) return 1;
    for (const auto& kv : session.lastKeyValues()) {
        std::cout << kv.key << " => " << kv.value << "\n";
    }

    auto del = session.del("/demo/key");
    if (!del) return 1;

    session.close();
    return 0;
}
```

## 3. 异步客户端最小示例

```cpp
#include "galay-etcd/async/AsyncEtcdClient.h"
#include <galay-kernel/kernel/Runtime.h>

#include <atomic>
#include <iostream>
#include <thread>

using galay::kernel::Coroutine;
using galay::kernel::IOScheduler;
using galay::kernel::Runtime;

Coroutine run(IOScheduler* scheduler, std::atomic<bool>* done) {
    galay::etcd::EtcdConfig cfg;
    cfg.endpoint = "http://127.0.0.1:2379";

    galay::etcd::AsyncEtcdClient client(scheduler, cfg);

    auto conn = co_await client.connect();
    if (!conn) {
        std::cerr << "连接失败: " << conn.error().message() << '\n';
        done->store(true);
        co_return;
    }

    auto put = co_await client.put("/demo/async", "value");
    if (!put) {
        std::cerr << "写入失败: " << put.error().message() << '\n';
        co_await client.close();
        done->store(true);
        co_return;
    }

    auto get = co_await client.get("/demo/async");
    if (!get) {
        std::cerr << "读取失败: " << get.error().message() << '\n';
        co_await client.close();
        done->store(true);
        co_return;
    }

    for (const auto& kv : client.lastKeyValues()) {
        std::cout << kv.key << " => " << kv.value << '\n';
    }

    (void)co_await client.del("/demo/async");
    (void)co_await client.close();
    done->store(true);
}

int main() {
    Runtime runtime(1, 1);
    runtime.start();

    auto* scheduler = runtime.getNextIOScheduler();
    if (!scheduler) {
        runtime.stop();
        return 1;
    }

    std::atomic<bool> done{false};
    scheduler->spawn(run(scheduler, &done));

    while (!done.load(std::memory_order_acquire)) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }

    runtime.stop();
    return 0;
}
```
