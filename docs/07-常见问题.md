...
2. 检测连接断开后重连：
   ...
   if (!put && put.error().type() == EtcdErrorType::Connection) {
       ...
       if (auto reconnect = co_await client.connect(); !reconnect) {
           std::cerr << "重连失败\n";
       }
   }
   ```
...
1. key 是否正确（注意前导斜杠）：
   ```cpp
   if (auto put = co_await client.put("/key", "value"); !put) {
       std::cerr << "put 失败\n";
   }
   if (auto get = co_await client.get("/key"); !get) {
       std::cerr << "get 失败\n";
   }

   if (auto put = co_await client.put("key", "value"); !put) {   // 不推荐
       std::cerr << "put 失败\n";
   }
   if (auto get = co_await client.get("/key"); !get) {           // 找不到
       std::cerr << "get 失败\n";
   }
   ```
...
2. 是否使用了租约且租约已过期：
   ...
   if (auto put = co_await client.put("/key", "value", client.lastLeaseId()); !put) {
       std::cerr << "put 失败\n";
   }
   std::this_thread::sleep_for(std::chrono::seconds(6));
   if (auto get = co_await client.get("/key"); !get) {  // 键已过期
       std::cerr << "get 失败\n";
   }
   ```
...
```cpp
// 写入
if (auto __await_result = co_await client.put("/app/config/db/host", "localhost"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.put("/app/config/db/port", "3306"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 正确：前缀查询
if (auto __await_result = co_await client.get("/app/config/db/", true); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 返回 2 个键

// 错误：前缀不匹配
if (auto __await_result = co_await client.get("/app/config/", true); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 返回 2 个键
if (auto __await_result = co_await client.get("/app/config/db", true); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
...
```cpp
// 正确
if (auto __await_result = co_await client.grantLease(5); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 5 秒
if (auto __await_result = co_await client.grantLease(60); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 60 秒

// 错误
if (auto __await_result = co_await client.grantLease(0); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // TTL 必须 > 0
if (auto __await_result = co_await client.grantLease(-1); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // TTL 必须 > 0
```
...
1. 租约是否正确绑定：
   ...
   if (auto put = co_await client.put("/key", "value", lease_id); !put) {  // 必须传递 lease_id
       std::cerr << "put 失败\n";
   }
   ```
...
2. 是否在续约：
   ```cpp
   // 如果持续续约，键不会过期
   if (auto keepalive = co_await client.keepAliveOnce(lease_id); !keepalive) {
       std::cerr << "续约失败\n";
   }
   ```
...
1. 使用 Pipeline 批量操作：
   ...
   // 低效
   for (int i = 0; i < 100; ++i) {
       if (auto put = co_await client.put("/key" + std::to_string(i), "value"); !put) {
           std::cerr << "put 失败\n";
       }
   }
   ...
   if (auto piped = co_await client.pipeline(std::move(ops)); !piped) {
       std::cerr << "pipeline 失败\n";
   }
   ```
...
2. 前缀查询使用 limit：
   ```cpp
   if (auto get = co_await client.get("/prefix/", true, 100); !get) {  // 限制返回 100 条
       std::cerr << "get 失败\n";
   }
   ```
...
if (auto __await_result = co_await client.put("/key", "value"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
