...
```cpp
// 低效：多次网络往返
if (auto __await_result = co_await client.put("/key1", "value1"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.put("/key2", "value2"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.put("/key3", "value3"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
if (auto __await_result = co_await client.pipeline(std::move(ops)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
if (auto __await_result = co_await client.connect(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 多次操作复用同一连接
for (int i = 0; i < 100; ++i) {
    if (auto __await_result = co_await client.put("/key" + std::to_string(i), "value"); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
...
config.request_timeout = std::chrono::seconds(5);  // 统一请求超时
...
public:
    ...
    Coroutine start() {
        if (auto __await_result = co_await m_client->connect(); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
    ...
    Coroutine stop() {
        std::string key = "/services/" + m_service_name + "/" + m_service_addr;
        if (auto __await_result = co_await m_client->del(key); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
...
public:
    ...
    Coroutine unlock() {
        ...
        std::string lock_key = "/locks/" + m_lock_name;
        if (auto __await_result = co_await m_client->del(lock_key); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
        m_lease_id = 0;
        co_await m_client->close();
...
Coroutine useLock(IOScheduler* scheduler) {
    ...
    if (auto __await_result = co_await lock.unlock(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    std::cout << "释放锁成功\n";
}
...
if (auto __await_result = co_await client.put("/key", "value", client.lastLeaseId()); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
if (auto __await_result = co_await client.put("/key", "value", lease_id); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
scheduler->spawn([&client, lease_id]() -> Coroutine {
    while (true) {
        ...
        if (auto __await_result = co_await client.keepAliveOnce(lease_id); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
    }
}());
...
```cpp
// 错误：前缀查询可能返回大量数据
if (auto __await_result = co_await client.get("/prefix/", true); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 可能返回数万条记录

// 正确：设置 limit
if (auto __await_result = co_await client.get("/prefix/", true, 100); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 最多返回 100 条
```
